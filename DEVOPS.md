...
- В ответ придет `projectId`, записываем его в `devops/env.ts`. Дальнейшие стадии используют его.
...
Запускаем SST напрямую `bun sst remove --stage {название окружения}`. Если нужно удалить все, то удаляем и `init`. Конечно, удалиться все на свете - БД, бакет с файлами и т.д.
